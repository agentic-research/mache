...
- **Question.** Of the edits an agent makes, what fraction introduce a structural
  (tree-sitter-parseable) syntax error that mache rejects at the write boundary —
  i.e. corruption mache prevents that a plain filesystem would let land?
- **Input.** A corpus of *real agent edit trajectories* (Write/Edit operations
  with resulting file content) — minable from Claude Code transcripts via lectio.
- **Oracle.** Objective and mache-independent: a file tree-sitter-**parses or it
  doesn't** (`internal/writeback/validate.go`). No judgment.
- **Measure.** Replay each edit's resulting file through the validator; count
  rejects. Cross-reference: of rejected edits, how many *did* land in the real
  trajectory and cost a later build-failure cycle.
- **Output.** catch-rate % + estimated wasted-CI-cycles avoided.
- **Baseline.** Plain filesystem (every edit lands; failures surface at build).
- **Falsification.** catch-rate ≈ 0 ⇒ the "red squiggly before save" claim is
  hollow. Pre-register a threshold (e.g. ≥X% of real broken edits caught).
- **Status.** **First dispatchable.** No mount, no `context`, objective oracle.
  Gap = the edit-trajectory extraction + replay harness (small).
...
1. **M1** — edit-trajectory corpus + replay-through-validator. Cheapest real number.
1. **M2** — after `mache-b8fe72` (context projection) lands.
1. **M3/M4** — SWE-bench Verified harness. The headline; the build.
