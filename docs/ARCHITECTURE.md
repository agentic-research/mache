...
| UDS query proxy               | `cmd/uds_graph.go`, `cmd/serve.go::buildControlGraph`                 | `udsGraph` implements `graph.Graph` over a UDS socket to the ley-line daemon. Used today by `serve --control`. The daemon owns SQLite (zero-copy via `sqlite3_deserialize` on the arena); mache never opens SQLite on this path. `mount --control` still uses `SQLiteGraph` + `ExtractActiveDB` pending daemon-side `find_callees` op (LLO Bead B / mache-98b9bf gating). |
