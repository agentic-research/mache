...
- **`Walker` interface** — Abstracts over query engines. `JsonWalker` uses JSONPath; `ASTWalker` queries the leyline `_ast` table via SQL (pure Go — in-process tree-sitter was removed in ADR-0012 step 4). Both return `Match` results with captured values and optional recursion context.
...
Mache is pure Go and pairs with [ley-line-open](https://github.com/agentic-research/ley-line-open) as its universal parser. Since [ADR-0012](adr/0012-cgo-removal-migration.md) step 4 (`mache-37ae8b`, v0.18.0), there is a **single** source path — the in-process CGO tree-sitter mode (SitterWalker) was removed:

```
source dir → leyline parse (pure-Go daemon, Rust tree-sitter) → .db with
             _ast / _source / node_refs / node_defs / node_content / _imports / _lsp* tables
                          ↓
...
| `find_smells` rules: `drift_doc_broken_internal_link`, `drift_doc_outdated_count`                                           | ✓                     | ✓                          | v1 placeholders (ADR-0018) — appear in the rule listing, currently return zero findings pending firing-logic follow-ups under mache-e1b6c8                                                                                                                      |
| `find_smells` rule: `drift_doc_dead_symbol_reference`                                                                       | ✗ (degrades to empty) | ✓                          | Real query (mache-eb2bf3) against a Go-probed `v_doc_refs` view — needs `node_refs.source_id`, which only leyline-native output populates. Rust-scoped for v1 (`token LIKE '%::%'`); advisory only, not gate-tagged                                             |
...
| AST queries (SQL over \_ast)  | `internal/ingest/ast_walker.go`                                                     | `ASTWalker.Query`, `astMatch.CaptureOrigin` — leyline `_ast`, pure Go                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [0012: CGO Removal Migration](adr/0012-cgo-removal-migration.md)                         | Accepted — FULLY SHIPPED (mache-37ae8b, v0.18.0) | leyline is the universal parser; SitterWalker + tree-sitter dep removed; CGO_ENABLED=0                                                                                                                                                                                                                                                                                                                                                        |
