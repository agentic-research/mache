...
| UDS query proxy               | `cmd/uds_graph.go`, `cmd/serve.go::buildControlGraph`, `cmd/mount.go::mountControl` | `udsGraph` implements `graph.Graph` over a UDS socket to the ley-line daemon. The daemon owns SQLite (zero-copy via `sqlite3_deserialize` on the arena); mache never opens SQLite in `--control` read-only mode. Replaces the legacy `ExtractActiveDB` temp-file copy. Hot-swap is implicit (each query reflects the daemon's current snapshot). Writable mount still uses `ExtractActiveDB` because the WritableGraph needs an in-process SQL connection for splice-on-close. |
