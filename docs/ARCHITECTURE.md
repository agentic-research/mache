...
The `.capnp` schemas live in `schemas/` (vendored from ley-line-open with three Go-annotation lines added; otherwise byte-identical). Generated Go bindings live in `internal/lsp/bindings/`. Regenerate with `task gen-bindings` (pinned to the `capnpc-go` version in `go.mod`).
...
| `find_smells` rules: `magic_int_in_comparison`, `cyclomatic_complexity`, `long_function`, `long_file`      | ✗                | ✓                          | Need `_ast` table — only ley-line-open writes it                                                                                                                                                                                                                |
...
| MCP server                    | `cmd/serve.go`, `cmd/serve_handlers.go`, `cmd/serve_registry.go` | `mache serve` (entry); `registerMCPTools` registers 17 tool handlers; `lazyGraph` is in serve_registry.go; `buildServeGraph` orchestrates ingestion                                                         |
...
| Vendored capnp schemas        | `schemas/{common,binding}.capnp`                                 | byte-identical to ley-line-open upstream + 3-line Go annotation block                                                                                                                                       |
...
| Control block                 | `internal/control/`                                              | HotSwapGraph, live schema reload. v0.8.0+: substrate identity is BLAKE3 `current_root` (was monotonic generation); paired with [ley-line-open](https://github.com/agentic-research/ley-line-open) ≥ v0.2.0. |
