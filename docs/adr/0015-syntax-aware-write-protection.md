# 15. Syntax-Aware Write Protection (Strict Mode)

Date: 2026-02-13 (renumbered from ADR-0006 to resolve a numbering collision with `0006-pure-go-mcp-first.md`)

## Status

Accepted (the validate → format → splice pipeline is what `internal/writeback/` implements today; see ADR-0009 for the full write pipeline framing)
...
1. **Incremental Parse:** Mache invokes Tree-sitter to parse the *proposed* new content.
1. **Error Check:** We traverse the resulting AST for `ERROR` or `MISSING` nodes.
1. **Gate:**
...
1. Use `sitter.Parser.Parse` on the buffer.
1. Walk tree to check for `n.HasError()`.
1. Return `fuse.EIO` if error found and strict mode is on.
