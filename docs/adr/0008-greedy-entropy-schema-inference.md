---
title: "ADR-0008: Greedy Entropy-Based Schema Inference"
status: Implemented
date: 2026-02-14
tags: [architecture, schema-inference, entropy, git, implemented]
---
...
1. **Candidate Selection:** Identifying potential split attributes.
   - Fields with cardinality > 1.
   - Virtual fields derived from hints (e.g., `timestamp:year`, `timestamp:month`).
1. **Scoring:** Selecting the best attribute to split on by maximizing a weighted score:
...
1. **Partitioning:** Splitting records into subsets based on the chosen attribute.
1. **Recursion:** Repeating until max depth is reached, entropy is minimized, or record count is low.
...
1. **Cardinality:** Attributes with fewer distinct values (minimizing directory clutter).
