...
func registerMCPTools(s *server.MCPServer, r *graphRegistry) {
	...
	s.AddTool(
		mcp.NewTool("get_communities",
			readTool(),
			mcp.WithDescription("Find clusters of related code using Louvain modularity detection. Use for 'what code works together?', 'find subsystems'. Requires dense cross-references. Hub tokens (referenced by more than max_fan_in nodes — `err`, `t`, `len`) are pruned before projection and reported: they cost K(K-1)/2 edges each and join everything to everything, which both dominates runtime and collapses real clusters into one blob. Use summary=true plus limit for large codebases."),
			mcp.WithNumber("min_size", mcp.Description("Minimum community size (default 2)")),
			mcp.WithString("granularity",
				mcp.Description("Cell size: 'construct' (AST nodes, default), 'file', or 'dir'. File/dir cells answer 'which files/packages belong together' — the decomposition question — by aggregating along the fs hierarchy already encoded in every node id."),
			),
			mcp.WithString("scope",
				mcp.Description("Path prefix filter, e.g. 'cmd/'. Only nodes under this prefix participate."),
			),
			mcp.WithNumber("max_fan_in",
				mcp.Description("Prune tokens referenced by more than this many nodes before projecting (default 100, 0 disables). Measured on a 50k-line Go repo: 100 keeps 98.5% of tokens, removes 99.2% of edge work, and raises modularity from 0.543 to 0.985."),
			),
			mcp.WithNumber("limit",
				mcp.Description("Maximum communities returned, largest first (default 50, 0 for all). Detection routinely finds thousands; returning them all overruns the response budget."),
			),
