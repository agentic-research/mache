...
while IFS= read -r sha; do
  ...
  if printf '%s' "$trailers" | grep -qiE "^Changelog:[[:space:]]*none[[:space:]]*$"; then
    continue
  fi
  ...
  while IFS= read -r id; do
    [ -n "$id" ] || continue
    # A bash substring test, NOT `printf | grep -q`. `grep -q` exits on the
    # first match and closes the pipe; printf still has the rest of the
    # section to write, takes EPIPE, and `set -o pipefail` makes the whole
    # pipeline non-zero — so `! pipeline` reported the id MISSING precisely
    # because it was FOUND. Whether printf finishes first depends on the pipe
    # buffer, so it passed on macOS (64 KiB, section fits) and inverted on the
    # Linux CI runner. This form has no pipe, no subprocess and no timing.
    case "$UNRELEASED" in
    *"$id"*) continue ;;
    esac
    if true; then
