...
// FindSmellsOptions parameterizes a programmatic find-smells run — the same
// engine the CLI flags drive, as a struct. Zero values take the CLI defaults
// (Limit 200, Format json, FailOn error).
type FindSmellsOptions struct {
	DB, Rule, SourceID, Format, FailOn, Tags, RulesDir string
...
// RunFindSmells executes one find-smells run from opts and returns the
// intended process exit code. It drives the same package-level command state
// the CLI does (snapshotted and restored), so a run through here and a run
// through the flags are the same code path — which is exactly what the
// MCP↔CLI byte-parity gate needs to stay a single assertion. Not safe for
// concurrent use, same as the CLI singleton it wraps.
func RunFindSmells(opts FindSmellsOptions) (int, error) {
	type snap struct {
		db, rule, src, format, failOn, tags, rulesDir, baseline, writeBaseline, baselineRoot string
		limit                                                                                int
		minMetric                                                                            int64
	}
	saved := snap{findSmellsDBPath, findSmellsRule, findSmellsSourceID, findSmellsFormat, findSmellsFailOn, findSmellsTags, findSmellsRulesDir, findSmellsBaseline, findSmellsWriteBaseline, findSmellsBaselineRoot, findSmellsLimit, findSmellsMinMetric}
	defer func() {
		findSmellsDBPath, findSmellsRule, findSmellsSourceID, findSmellsFormat, findSmellsFailOn, findSmellsTags, findSmellsRulesDir, findSmellsBaseline, findSmellsWriteBaseline, findSmellsBaselineRoot, findSmellsLimit, findSmellsMinMetric = saved.db, saved.rule, saved.src, saved.format, saved.failOn, saved.tags, saved.rulesDir, saved.baseline, saved.writeBaseline, saved.baselineRoot, saved.limit, saved.minMetric
	}()

	findSmellsDBPath, findSmellsRule, findSmellsSourceID = opts.DB, opts.Rule, opts.SourceID
	findSmellsTags, findSmellsRulesDir = opts.Tags, opts.RulesDir
	findSmellsBaseline, findSmellsWriteBaseline, findSmellsBaselineRoot = opts.Baseline, opts.WriteBaseline, opts.BaselineRoot
	findSmellsMinMetric = opts.MinMetric
	findSmellsFormat = opts.Format
	if findSmellsFormat == "" {
		findSmellsFormat = "json"
	}
	findSmellsFailOn = opts.FailOn
	if findSmellsFailOn == "" {
		findSmellsFailOn = "error"
	}
	findSmellsLimit = opts.Limit
	if findSmellsLimit <= 0 {
		findSmellsLimit = 200
	}

	if opts.Out != nil {
		findSmellsCmd.SetOut(opts.Out)
	}
	if opts.Err != nil {
		findSmellsCmd.SetErr(opts.Err)
	}
	return runFindSmells(findSmellsCmd, nil)
}

// FindSmellsCmd returns the find-smells cobra command for registration on the
// root command. Registration hook #1 of the cmd decomposition (mache-96c378):
// the package defines the command; cmd/register.go decides it exists on the
// CLI. smells never imports cmd.
func FindSmellsCmd() *cobra.Command { return findSmellsCmd }
