...
// The Unreleased section in this repo is ~25 KiB and growing. This case makes
// it larger than a pipe buffer with the matching id near the TOP, which is what
// the original implementation could not survive: it tested for the id with
// `printf '%s' "$UNRELEASED" | grep -qF "$id"`, and `grep -q` exits on the first
// match, closing the pipe while printf still has the rest to write. printf takes
// EPIPE, `set -o pipefail` propagates it, and `!` turns that into "missing" —
// so the gate failed precisely when the entry EXISTED.
//
// It is timing- and buffer-dependent, which is why every other case here passed
// on macOS and the gate inverted on the Linux CI runner. A small fixture cannot
// reach it; the size is the test.
func TestChangelogEntryCheck_LargeUnreleasedSectionWithAnEarlyMatch(t *testing.T) {
	var b strings.Builder
	b.WriteString("# Changelog\n\n## [Unreleased]\n\n### Fixed\n\n")
	b.WriteString("- The entry under review (`mache-aaaaaa`).\n\n")
	// Well past the 64 KiB pipe buffer, so printf cannot finish before a
	// short-circuiting reader closes the pipe.
	for i := 0; i < 4000; i++ {
		b.WriteString("- Filler entry number ")
		b.WriteString(strconv.Itoa(i))
		b.WriteString(", padded so the section comfortably exceeds a pipe buffer.\n")
	}
	b.WriteString("\n## [v1.0.0] — 2026-01-01\n\n- Shipped (`mache-bbbbbb`).\n")
	changelog := b.String()
	require.Greater(t, len(changelog), 64*1024,
		"the fixture must exceed a pipe buffer or it cannot reach the defect")

	out, code := runCase(t, changelog, "[mache-aaaaaa] fix(thing): a user-facing fix")
	assert.Equal(t, 0, code,
		"the id IS in Unreleased; reporting it missing is the EPIPE inversion. output:\n%s", out)
}
